...
void Router::rxProcess()
{
	...
	for (int i = 0; i < DIRECTIONS + 2; i++) {
		...
		if (!buffer[i][vc].IsFull()) 
		{
			...
			if (now < 40 && local_id == 0 && i == DIRECTION_LOCAL) {
				cout << "[R0 RX_LOCAL] cycle=" << now
					<< " dst=" << received_flit.dst_id
					<< " vc=" << vc
					<< " type=" << received_flit.flit_type
					<< endl;
			}

			if (now < 80 && local_id == 11 && i != DIRECTION_LOCAL) {
				cout << "[R11 RX_NET] cycle=" << now
					<< " in=" << i
					<< " dst=" << received_flit.dst_id
					<< " vc=" << vc
					<< " type=" << received_flit.flit_type
		  ...
		  if (flit.flit_type == FLIT_TYPE_HEAD) 
		    {
			  ...
			  if (now < 120 && flit.dst_id == local_id) {
				  cout << "[ROUTE_LOCAL] router=" << local_id
					  << " cycle=" << now
					  << " in=" << i
					  << " dst=" << flit.dst_id
					  << " chosen_out=" << o
	  ...
	  if (reservations.size()!=0)
	  {
	     ...
	     // LOG<< "found reservation from input= " << i << "_to output= "<<o<<endl;
		       ...
		       (buffer_full_status_tx[o].read().mask[vc] == false) ) 
		  {
			  ...
			  if (now < 120 && flit.dst_id == local_id) {
				  cout << "[FWD_LOCAL_OK] router=" << local_id
					  << " cycle=" << now
					  << " in=" << i
					  << " out=" << o
					  << " ack=" << ack_tx[o].read()
					  << " cur=" << current_level_tx[o]
					  << " bfs=" << buffer_full_status_tx[o].read().mask[vc]
					  << " type=" << flit.flit_type
					  << endl;
			  }
			  */
			  ...
			  ///////////
			  /*
			  if (now < 40 && local_id == 0) {
				  cout << "[R0 FWD] cycle=" << now
					  << " in=" << i
					  << " out=" << o
					  << " dst=" << flit.dst_id
					  << " type=" << flit.flit_type
			  ...
			  */
			  ...
			  if (flit.src_id == 0 && flit.dst_id == 12) {
				  cout << "[CIRC_LOCAL_PATH_OK] cycle=" << now
					  << " router=" << local_id
					  << " in=" << i
					  << " out=" << o
					  << " vc=" << vc
		  ...
		  else
		  {
			  ...
			  if (now < 120 && flit.dst_id == local_id) {
				  cout << "[FWD_LOCAL_BLOCK] router=" << local_id
					  << " cycle=" << now
					  << " in=" << i
					  << " out=" << o
					  << " ack=" << ack_tx[o].read()
			 ...
			 double now = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
			  ...
			  if (flit.src_id == 0 && flit.dst_id == 12) {
				  cout << "[CIRC_LOCAL_PATH_BLOCK] cycle=" << now
					  << " router=" << local_id
					  << " in=" << i
					  << " out=" << o
					  << " vc=" << vc
...
{
    if (GlobalParams::topology == TOPOLOGY_CIRCULANT)
    {
