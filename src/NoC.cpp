...
void NoC::buildCirculant()
{
    ...
    // One signal bundle per node is enough for the first circulant version
    const int dimX = n;
    const int dimY = 1;
    ...
    // Dummy signals for the first version: NoP and free_slots are not used yet
    sc_signal<bool>* bool_dummy_signal = new sc_signal<bool>;
    sc_signal<int>* int_dummy_signal = new sc_signal<int>;
    sc_signal<Flit>* flit_dummy_signal = new sc_signal<Flit>;
    sc_signal<NoP_data>* nop_data_dummy_signal = new sc_signal<NoP_data>;
    ...
    for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {
        for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
            ...
            char tile_name[64];
            int tile_id = j * GlobalParams::mesh_dim_x + i;
            ...
            const int plus1  = (tile_id + 1) % n;
            const int minus1 = (tile_id - 1 + n) % n;
            const int pluss  = (tile_id + s) % n;
            const int minuss = (tile_id - s + n) % n;

			// Rx signals
			t[i][j]->req_rx[DIRECTION_NORTH](req[plus1][0].south);
			t[i][j]->flit_rx[DIRECTION_NORTH](flit[plus1][0].south);
			t[i][j]->ack_rx[DIRECTION_NORTH](ack[plus1][0].south);
			t[i][j]->buffer_full_status_rx[DIRECTION_NORTH](buffer_full_status[plus1][0].south);

			t[i][j]->req_rx[DIRECTION_EAST](req[pluss][0].west);
			t[i][j]->flit_rx[DIRECTION_EAST](flit[pluss][0].west);
			t[i][j]->ack_rx[DIRECTION_EAST](ack[pluss][0].west);
			t[i][j]->buffer_full_status_rx[DIRECTION_EAST](buffer_full_status[pluss][0].west);

			t[i][j]->req_rx[DIRECTION_SOUTH](req[minus1][0].north);
			t[i][j]->flit_rx[DIRECTION_SOUTH](flit[minus1][0].north);
			t[i][j]->ack_rx[DIRECTION_SOUTH](ack[minus1][0].north);
			t[i][j]->buffer_full_status_rx[DIRECTION_SOUTH](buffer_full_status[minus1][0].north);

			t[i][j]->req_rx[DIRECTION_WEST](req[minuss][0].east);
			t[i][j]->flit_rx[DIRECTION_WEST](flit[minuss][0].east);
			t[i][j]->ack_rx[DIRECTION_WEST](ack[minuss][0].east);
			t[i][j]->buffer_full_status_rx[DIRECTION_WEST](buffer_full_status[minuss][0].east);

			// Tx signals
			t[i][j]->req_tx[DIRECTION_NORTH](req[tile_id][0].north);
			t[i][j]->flit_tx[DIRECTION_NORTH](flit[tile_id][0].north);
			t[i][j]->ack_tx[DIRECTION_NORTH](ack[tile_id][0].north);
			t[i][j]->buffer_full_status_tx[DIRECTION_NORTH](buffer_full_status[tile_id][0].north);

			t[i][j]->req_tx[DIRECTION_EAST](req[tile_id][0].east);
			t[i][j]->flit_tx[DIRECTION_EAST](flit[tile_id][0].east);
			t[i][j]->ack_tx[DIRECTION_EAST](ack[tile_id][0].east);
			t[i][j]->buffer_full_status_tx[DIRECTION_EAST](buffer_full_status[tile_id][0].east);

			t[i][j]->req_tx[DIRECTION_SOUTH](req[tile_id][0].south);
			t[i][j]->flit_tx[DIRECTION_SOUTH](flit[tile_id][0].south);
			t[i][j]->ack_tx[DIRECTION_SOUTH](ack[tile_id][0].south);
			t[i][j]->buffer_full_status_tx[DIRECTION_SOUTH](buffer_full_status[tile_id][0].south);

			t[i][j]->req_tx[DIRECTION_WEST](req[tile_id][0].west);
			t[i][j]->flit_tx[DIRECTION_WEST](flit[tile_id][0].west);
			t[i][j]->ack_tx[DIRECTION_WEST](ack[tile_id][0].west);
			t[i][j]->buffer_full_status_tx[DIRECTION_WEST](buffer_full_status[tile_id][0].west);


            // Hub connections, same idea as in buildMesh()
            t[i][j]->hub_req_rx(req[tile_id][0].from_hub);
            t[i][j]->hub_flit_rx(flit[tile_id][0].from_hub);
            t[i][j]->hub_ack_rx(ack[tile_id][0].to_hub);
            t[i][j]->hub_buffer_full_status_rx(buffer_full_status[tile_id][0].to_hub);

            t[i][j]->hub_req_tx(req[tile_id][0].to_hub);
            t[i][j]->hub_flit_tx(flit[tile_id][0].to_hub);
            t[i][j]->hub_ack_tx(ack[tile_id][0].from_hub);
            t[i][j]->hub_buffer_full_status_tx(buffer_full_status[tile_id][0].from_hub);
            ...
            if (it != GlobalParams::hub_for_tile.end()) {
                ...
                hub[hub_id]->req_rx[port](req[tile_id][0].to_hub);
                hub[hub_id]->flit_rx[port](flit[tile_id][0].to_hub);
                hub[hub_id]->ack_rx[port](ack[tile_id][0].from_hub);
                hub[hub_id]->buffer_full_status_rx[port](buffer_full_status[tile_id][0].from_hub);

                hub[hub_id]->flit_tx[port](flit[tile_id][0].from_hub);
                hub[hub_id]->req_tx[port](req[tile_id][0].from_hub);
                hub[hub_id]->ack_tx[port](ack[tile_id][0].to_hub);
                hub[hub_id]->buffer_full_status_tx[port](buffer_full_status[tile_id][0].to_hub);
            }
			// Real free_slots / NoP connections for circulant neighbors

			t[i][j]->free_slots[DIRECTION_NORTH](free_slots[tile_id][0].north);
			t[i][j]->free_slots[DIRECTION_EAST] (free_slots[tile_id][0].east);
			t[i][j]->free_slots[DIRECTION_SOUTH](free_slots[tile_id][0].south);
			t[i][j]->free_slots[DIRECTION_WEST] (free_slots[tile_id][0].west);

			t[i][j]->free_slots_neighbor[DIRECTION_NORTH](free_slots[plus1][0].south);
			t[i][j]->free_slots_neighbor[DIRECTION_EAST] (free_slots[pluss][0].west);
			t[i][j]->free_slots_neighbor[DIRECTION_SOUTH](free_slots[minus1][0].north);
			t[i][j]->free_slots_neighbor[DIRECTION_WEST] (free_slots[minuss][0].east);

			t[i][j]->NoP_data_out[DIRECTION_NORTH](nop_data[tile_id][0].north);
			t[i][j]->NoP_data_out[DIRECTION_EAST] (nop_data[tile_id][0].east);
			t[i][j]->NoP_data_out[DIRECTION_SOUTH](nop_data[tile_id][0].south);
			t[i][j]->NoP_data_out[DIRECTION_WEST] (nop_data[tile_id][0].west);

			t[i][j]->NoP_data_in[DIRECTION_NORTH](nop_data[plus1][0].south);
			t[i][j]->NoP_data_in[DIRECTION_EAST] (nop_data[pluss][0].west);
			t[i][j]->NoP_data_in[DIRECTION_SOUTH](nop_data[minus1][0].north);
			t[i][j]->NoP_data_in[DIRECTION_WEST] (nop_data[minuss][0].east);
...
Tile *NoC::searchNode(const int id) const
{
    if (GlobalParams::topology == TOPOLOGY_MESH ||
        GlobalParams::topology == TOPOLOGY_CIRCULANT)
    {
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++)
	    for (int j = 0; j < GlobalParams::mesh_dim_y; j++)
