...
void loadConfiguration() {
    ...
    // Circulant network params
    if (GlobalParams::topology == TOPOLOGY_CIRCULANT) {
        GlobalParams::mesh_dim_x = readParam<int>(config, "mesh_dim_x");
        GlobalParams::mesh_dim_y = readParam<int>(config, "mesh_dim_y");
        GlobalParams::circulant_step = readParam<int>(config, "circulant_step");
    }
...
void checkConfiguration()
{
    ...
    else if (GlobalParams::topology == TOPOLOGY_CIRCULANT)
    {
        if (GlobalParams::mesh_dim_x <= 1) {
            cerr << "Error: dimx must be greater than 1" << endl;
            exit(1);
        ...
        int n = GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y;
        int s = GlobalParams::circulant_step;

        if (s <= 1 || s >= n - 1) {
            cerr << "Error: circulant_step must satisfy 1 < circulant_step < N-1, where N = mesh_dim_x * mesh_dim_y" << endl;
            exit(1);
        }
        ...
        if (GlobalParams::routing_algorithm != ROUTING_CIRCULANT)
        {
            cerr << "Error: CIRCULANT topology requires CIRCULANT routing algorithm" << endl;
            exit(1);
        }
    }
    ...
    {
        cerr << "Error: unsupported topology " << GlobalParams::topology << endl;
        exit(1);
    }
