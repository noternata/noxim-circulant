...
void ProcessingElement::txProcess()
{
    ...
    if(GlobalParams::traffic_distribution != TRAFFIC_HARDCODED) {
        ...
        if (canShot(packet)) {
            ...
            if (local_id == 0 && now < 30) {
                cout << "[PE0 GEN] cycle=" << now
                    << " dst=" << packet.dst_id
                    << " size=" << packet.size
    ...
    if (ack_tx.read() == current_level_tx) {
        ...
        if (local_id == 0 && now < 30 && !packet_queue.empty()) {
            cout << "[PE0 WAIT_ACK] cycle=" << now
                << " ack=" << ack_tx.read()
                << " cur=" << current_level_tx
                << " qsize=" << packet_queue.size()
                << endl;
        }
    }
